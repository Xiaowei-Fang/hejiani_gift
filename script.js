...
const MESSAGE_EMAIL = "506582374@qq.com";
const MESSAGE_ENDPOINT = `https://formsubmit.co/ajax/${MESSAGE_EMAIL}`;
const MESSAGE_FALLBACK_ENDPOINT = `https://formsubmit.co/${MESSAGE_EMAIL}`;
...
function initMessageBoard() {
  ...
  form.addEventListener("submit", async (event) => {
    ...
    if (!message) {
      status.textContent = "先写一点悄悄话，我再帮你寄出去。";
    ...
    status.textContent = "正在把这封小信送出去...";

    try {
      const payload = {
        _subject: `小贺儿的悄悄话 - ${getBeijingCycleLabel()}`,
        _template: "box",
        _captcha: "false",
      ...
      status.textContent = result.message?.includes("activate")
        ? "已经提交啦。第一次使用时，我需要先在邮箱里确认一次收件权限。"
        : "已经寄出啦，我会在邮箱里收到这封小信。";
    } catch (error) {
      console.warn("留言发送失败。", error);
      if (error instanceof TypeError) {
        submitMessageWithPageFallback(message);
        return;
      }
      ...
      submitButton.textContent = "寄出这封小信";
    }
  });
}

function submitMessageWithPageFallback(message) {
  const fallbackForm = document.createElement("form");
  fallbackForm.method = "POST";
  fallbackForm.action = MESSAGE_FALLBACK_ENDPOINT;
