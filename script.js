...
async function initPhotoCube() {
  const stage = $("#photoCubeStage");
  const cube = $("#photoCube");
  const subtitle = $("#gallerySubtitle");
  const caption = $("#cubeCaption");
  const hint = $("#cubeHint");
  const zoomIn = $("#cubeZoomIn");
  const zoomOut = $("#cubeZoomOut");
  const resetButton = $("#cubeReset");
  ...
  const loadDailyPhotos = async () => {
    ...
    cubeState.dailyPhotos = dailyPhotos;
    renderCubeFaces();
    syncCaption();
    subtitle.textContent = `今天随机装进魔方的 6 张照片，点一点、转一转都可以。`;
  };
  ...
  const renderCubeFaces = () => {
    ...
    const faces = [
      ...
      { name: "left", label: "左面" },
      { name: "top", label: "上面" },
      { name: "bottom", label: "下面" }
    ];

    faces.forEach((face, index) => {
      const faceEl = document.createElement("button");
      faceEl.type = "button";
      faceEl.className = `cube-face face-${face.name}`;
      faceEl.dataset.faceIndex = String(index);
      faceEl.setAttribute("aria-label", `查看${face.label}的照片`);

      const image = document.createElement("img");
      image.src = `./assets/photos/${encodeURIComponent(dailyPhotos[index] || PHOTO_FILES[index % PHOTO_FILES.length])}`;
      image.alt = `小贺儿今天的魔方照片第 ${index + 1} 张`;
      image.loading = index === 0 ? "eager" : "lazy";
      image.addEventListener("load", () => faceEl.classList.add("has-photo"));
      image.addEventListener("error", () => faceEl.classList.add("photo-missing"));

      const content = document.createElement("span");
      content.className = "cube-face-label";
      content.textContent = faceNames[index];
      faceEl.append(image, content);

      faceEl.addEventListener("click", (event) => {
        if (cubeState.ignoreFaceClick) {
          event.preventDefault();
          return;
        }
        rotateToFace(index);
  ...
  const applyTransform = (animate = true) => {
    cube.style.transitionDuration = animate ? "380ms" : "0ms";
    cube.style.transform = `translate3d(0, 0, 0) rotateX(${state.rotateX}deg) rotateY(${state.rotateY}deg) scale(${state.scale})`;
    stage.style.setProperty("--cube-scale", String(state.scale));
  };

  const syncCaption = () => {
    const faceLabel = faceNames[state.activeFace] || "正面";
    caption.textContent = `当前朝前的是${faceLabel}，拖动就能转动魔方。`;
  };
  ...
  const startAutoSpin = () => {
    ...
    const tick = () => {
      ...
      autoSpinFrame = window.requestAnimationFrame(tick);
    };
  ...
  const updateActiveFace = () => {
    const absX = ((Math.round(state.rotateX / 90) % 4) + 4) % 4;
    const absY = ((Math.round(state.rotateY / 90) % 4) + 4) % 4;
    if (Math.abs(state.rotateX) > Math.abs(state.rotateY)) {
      state.activeFace = (absX + 2) % 4;
    } else {
      state.activeFace = (absY + 4) % 4;
    }
    syncCaption();
  };

  const settleToGrid = () => {
    state.rotateX = Math.round(state.rotateX / 90) * 90;
    state.rotateY = Math.round(state.rotateY / 90) * 90;
    applyTransform(true);
    updateActiveFace();
  };
  ...
  const zoom = (delta) => {
    stopAutoSpin();
    state.scale = clamp(state.scale + delta, 0.72, 1.38);
    applyTransform(true);
    caption.textContent = `魔方现在放大到 ${Math.round(state.scale * 100)}%。`;
  };
  ...
  const rotateToFace = (index) => {
    ...
    const target = rotations[index] || rotations[0];
    state.rotateX = target.x;
    state.rotateY = target.y;
    state.activeFace = index;
    applyTransform(true);
    syncCaption();
  };
  ...
  const updateDrag = (dx, dy) => {
    state.rotateY += dx * 0.35;
    state.rotateX -= dy * 0.35;
    state.rotateX = clamp(state.rotateX, -132, 132);
    state.rotateY = normalizeAngle(state.rotateY);
    applyTransform(false);
    updateActiveFace();
  };
  ...
  const finishDrag = () => {
    if (!drag) return;
    let { velocityX, velocityY } = drag;
    const step = () => {
      state.rotateY += velocityX * 18;
      state.rotateX -= velocityY * 18;
      state.rotateX = clamp(state.rotateX, -132, 132);
      state.rotateY = normalizeAngle(state.rotateY);
      applyTransform(false);
      velocityX *= 0.92;
  ...
  const onPointerDown = (event) => {
    ...
    if (activePointers.size === 2) {
      ...
      lastTwoFinger = getTouchDistance(points);
      drag = null;
      return;
    }
  ...
  const onPointerMove = (event) => {
    ...
    if (activePointers.size >= 2) {
      ...
      if (!lastTwoFinger) {
        lastTwoFinger = distance;
        return;
      }

      const delta = distance - lastTwoFinger;
      lastTwoFinger = distance;
      if (Math.abs(delta) > 1) movedDuringPointer = true;
      state.scale = clamp(state.scale + delta * 0.002, 0.72, 1.38);
      applyTransform(false);
      caption.textContent = `魔方现在放大到 ${Math.round(state.scale * 100)}%。`;
      return;
    }
    ...
    if (Math.hypot(dx, dy) > 1.5) movedDuringPointer = true;
    drag.velocityX = dx / Math.max(1, now - drag.lastTime);
    drag.velocityY = dy / Math.max(1, now - drag.lastTime);
  ...
  const onPointerUp = (event) => {
    ...
    if (activePointers.size < 2) lastTwoFinger = null;
    if (movedDuringPointer) {
      cubeState.ignoreFaceClick = true;
      window.setTimeout(() => {
        cubeState.ignoreFaceClick = false;
      }, 120);
    }
  ...
  zoomIn.addEventListener("click", () => zoom(0.08));
  zoomOut.addEventListener("click", () => zoom(-0.08));
  resetButton.addEventListener("click", resetCube);

  window.addEventListener("resize", debounce(() => applyTransform(false), 160));
  ...
  hint.addEventListener("transitionend", () => {
    hint.textContent = "拖动旋转 · 双指缩放 · 双击复位";
  });
}
